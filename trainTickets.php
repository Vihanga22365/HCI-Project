<!DOCTYPE html>
<html lang="en" dir="ltr">
<head>
    <meta charset="utf-8">
  

    <!-- Bootstrap core CSS -->
    <!-- <link href="assets1/bootstrap/css/bootstrap.css" rel="stylesheet"> -->


    <!-- <link href="assets1/css/style.css" rel="stylesheet"> -->


    <style type="text/css">
        p {
              text-align: justify;
              text-justify: inter-word;
        }
    </style>

</head>
<body>

<?php include('header.php');?>

<main id="main">

<!-- ======= Breadcrumbs Section ======= -->
<section class="breadcrumbs">
  <div class="container">

    <div class="d-flex justify-content-between align-items-center">
      <ol>
        <li><a href="index.html">Home</a></li>
        <li>Train Tickets</li>
      </ol>
    </div>

  </div>
</section><!-- End Breadcrumbs Section -->

<section class="inner-page" style="padding:0;">
		

   
    <div class="main-container inner-page">
        <div class="container">
            <div class="section-content">
    
                </div>
               <div class="container-content">

                <div class="inner-box " >

                        <h2>

                        <div class="section-title" style="margin-top: 2%;">
                          <h2> Train Tickets </h2>
                        </div>

                        <p ><strong><h4> How its works?</h4> </strong></p>
                        <ul class="list-dot">
                          <li>The reservation system is open 30 days in advance, therefore we advice you to send your booking more than 30 before the travel date, this will allow us to secure your place as soon as the reservation opened, The earlier you booked, there is greater the chance of securing your seats, but you can send your booking until up to 2 days prior to travel date.</li><br>
                          <li>Once we received your booking, we will contact the service provider to process your booking and will send you the confirmation details how / where to collect your tickets, in the event there is no availability we will refund you fully.</li>

                        </ul>
                        <br>

                       <ol class="list-number" style="margin-left: 30px; font-size: 14px;  ">
                       <li>Please click on above "BOOK NOW" button and follow the onscreen instruction.</li></br>
                      <li>If you book in advance, we will e-mail you the confirmation / reservation number approximately 30 days prior to travel date, if your travel date is less than 30 days, then we will send you the confirmation within 48hrs of booking being received, if you haven't received this by then please contact us.</li></br>
                      <li>Make your way to the relevant train station (at least 30 minutes) prior to the train departure to collect your tickets</li></br>
                      <li>Provide your reservation number and passport to the RAILWAY STATION ticket counter to obtain the ticket(s) </li></br>
                      <li>Reservation must be minimum 2 days in advance of your desired travel dates. sending prior this does not mean that tickets are guarantee we will attempt to secure your tickets when the sales  open and inform you accordingly.</li></br>
                     </ol>

                     <p><strong><h4> Changes /Cancellations / Refunds</h4> </strong></p>
                     <p style="font-size: 14px; color: red;"><strong>
                     PLEASE NOTE THAT SRI LANKAN RAILWAY DOES NOT ACCEPT ANY CANCELLATIONS / REFUND OR DATE / TIME /CLASS /NAME OR ANY OTHER CHANGES. Therefore once you send us your booking or ask us to process with available option, no refund or cancellation possible. Please read the full terms and conditions of booking below.</p></strong>

                     <p style="font-size: 15px;">Please note that below fares are including tickets price + service charge + our booking fee and other costs involved to process your tickets,  which means fares are higher than the actual (SLR) ticket price. you can buy base price ticket (s) over the counter at the station.<p style="color: red;">All our bookings come with reserved seats (including 3rd class).</p>
                      
                     <p><strong><h4> Please note: All the bookings are subject to the availability of SLR. If we are unable to book your tickets due to the availability, we will refund you fully.  </h4> </strong></p>


                     
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-6 col-md-6 col-lg-6" align="center"><a href="terms.php" class="btn btn-primary" style="width: 60%;">Terms & Conditions</a></div>
                            <div class="col-sm-6 col-md-6 col-lg-6" align="center"><a href="" class="btn btn-primary" style="width: 60%;">FAQ</a></div>
                        </div>
                    </div>

                     <br>
                   


</br>
</br>
                    <p ><strong><h2>Available Routes</h2> </strong></p>


<div aria-multiselectable="true" role="tablist" id="accordion" class="panel-group faq-panel">
                        <div class="card">
                            <div id="headingOne" role="tab" class="card-header">
                                <h4 class="card-title">
                                    <a style="color: blue;" aria-controls="collapseOne" aria-expanded="true" href="#collapseOne"
                                       data-parent="#accordion" aria-expanded="true"  data-toggle="collapse" >
                                       COLOMBO FORT - PERADENIYA
                                    </a>
                                </h4>
                            </div>
                            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse show"
                                 id="collapseOne">
                                <div class="card-body">
                                <table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Depature</th>
      <th scope="col">Time</th>
      <th scope="col">Arrival</th>
      <th scope="col">Time</th>
      <th scope="col">Train no:</th>
      <th scope="col">Train type</th>
      <th scope="col">Class </th>
      <th scope="col">A / C</th>
      <th scope="col">Fare</th>
    </tr>
  </thead>
  <tbody>
    
  <tr>
    
      <td> Colombo fort</td>
      <td>	06:00 hrs</td>
      <td>Peradeniya</td>
      <td> 09:15 hrs</td>
      <td>1005</td>
      <td> Express	</td>
      <td>1st Class </td>
      <td> Yes	</td>
      <td>£11.00 </td>
    </tr>
    				
    <tr>
    
    <td> Colombo fort</td>
    <td>	06:45 hrs</td>
    <td>Peradeniya</td>
    <td>09:17 hrs</td>
    <td>1001</td>
    <td> Express	</td>
    <td>1st/2nd Class </td>
    <td> Yes/No	</td>
    <td>£10.00 </td>
  </tr>
    				
  <tr>
    
    <td> Colombo fort</td>
    <td>	07:00 hrs</td>
    <td>Peradeniya</td>
    <td>09:21 hrs</td>
    <td>1009</td>
    <td> Express	</td>
    <td>Observation </td>
    <td>No	</td>
    <td>£11.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>	08:30 hrs</td>
    <td>Peradeniya</td>
    <td>10:54 hrs</td>
    <td>1015</td>
    <td> Express	</td>
    <td>2nd Class </td>
    <td> No	</td>
    <td>£10.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>	08:30 hrs</td>
    <td>Peradeniya</td>
    <td>10:54 hrs</td>
    <td>1015</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>

  <tr>
    
    <td> Colombo fort</td>
    <td>	TBC</td>
    <td>Peradeniya</td>
    <td>12:25 hrs	</td>
    <td>1007</td>
    <td> Express	</td>
    <td></td>
    <td> No	</td>
    <td>£10.00 </td>
  </tr>

  <tr>
    
    <td> Colombo fort</td>
    <td>	09:45 hrs</td>
    <td>Peradeniya</td>
    <td>12:25 hrs</td>
    <td>1007</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> No	</td>
    <td>£11.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>	15.35 hrs</td>
    <td>Peradeniya</td>
    <td>17:56 hrs</td>
    <td>1029</td>
    <td> Express	</td>
    <td>Observation </td>
    <td> No	</td>
    <td>£11.00 </td>
  </tr>
  <tr>

  <tr>
    
    <td> Colombo fort</td>
    <td>	15.35 hrs</td>
    <td>Peradeniya</td>
    <td>17:56 hrs</td>
    <td>1029</td>
    <td> Express	</td>
    <td> 2nd Class </td>
    <td> No	</td>
    <td>£10.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>20.00 hrs</td>
    <td>Peradeniya</td>
    <td>22:52 hrs</td>
    <td>1045</td>
    <td> Express	</td>
    <td>Observation</td>
    <td> No</td>
    <td>£11.00 </td>
  </tr>

  <tr>
    
    <td> Colombo fort</td>
    <td>20.00 hrs</td>
    <td>Peradeniya</td>
    <td>22:52 hrs</td>
    <td>1045</td>
    <td> Express	</td>
    <td>2nd Class </td>
    <td> No	</td>
    <td>£10.00 </td>
  </tr>
  
  </tbody>
</table>



                                </div>
                            </div>
                        </div>

                        <div class="card">
                            <div id="headingTwo" role="tab" class="card-header">
                                <h4 class="card-title">
                                    <a style="color: blue" aria-controls="collapseTwo" aria-expanded="false" href="#collapseTwo"
                                       data-parent="#accordion" aria-expanded="true"  data-toggle="collapse"  class="collapsed">
                                        Colombo Front-Kandy
                                    </a>
                                </h4>
                            </div>
                            <div aria-labelledby="headingTwo" role="tabpanel" class="panel-collapse collapse"
                                 id="collapseTwo">
                                <div class="card-body">
                                <table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Depature</th>
      <th scope="col">Time</th>
      <th scope="col">Arrival</th>
      <th scope="col">Time</th>
      <th scope="col">Train no:</th>
      <th scope="col">Train type</th>
      <th scope="col">Class </th>
      <th scope="col">A / C</th>
      <th scope="col">Fare</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td>Kandy</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>2nd Class</td>
    <td>No	</td>
    <td>£10.00 </td>
  </tr>
          
  <tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Kandy</td>
  <td>09:31 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
          
<tr>
  
  <td> Colombo fort</td>
  <td>	07:00 hrs</td>
  <td>Kandy</td>
  <td>09:15 hrs</td>
  <td>1009</td>
  <td> Express	</td>
  <td>2nd Class </td>
  <td> Yes	</td>
  <td>£10.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	07:00 hrs</td>
  <td>Kandy</td>
  <td>09:31 hrs</td>
  <td>1009</td>
  <td> Observation	</td>
  <td>1st Class </td>
  <td> No	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Kandy</td>
  <td>11:03 hrs</td>
  <td>1015</td>
  <td> Express	</td>
  <td>2nd Class </td>
  <td> No	</td>
  <td>£10.00 </td>
</tr>

<tr>
  
  <td> Colombo fort</td>
  <td>08:30 hrs</td>
  <td>Kandy</td>
  <td>11:03 hrs</td>
  <td> 1015</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

<tr>
  
  <td> Colombo fort</td>
  <td>09:00 hrs</td>
  <td>Kandy</td>
  <td>11:31 hrs</td>
  <td>1031</td>
  <td style="color: red;"> (Sat & Sun Only)	</td>
  <td>Observation	</td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>10:35 hrs</td>
  <td>Kandy</td>
  <td>13:52 hrs</td>
  <td>1019</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	15:35 hrs</td>
  <td>Kandy</td>
  <td>18:06 hrs</td>
  <td>1029</td>
  <td> Express	</td>
  <td>2nd Class </td>
  <td> No	</td>
  <td>£10.00 </td>
</tr>

<tr>
  
  <td> Colombo fort</td>
  <td>	15:35 hrs</td>
  <td>Kandy</td>
  <td>18:06 hrs</td>
  <td>1029</td>
  <td> Observation	</td>
  <td>1st Class </td>
  <td> No	</td>
  <td>£11.00 </td>
</tr>

<tr>
  
  <td> Colombo fort</td>
  <td>	17:20 hrs</td>
  <td>Kandy</td>
  <td>20:03 hrs</td>
  <td>1033 FS</td>
  <td style="color: red;"> (Sat & Sun Only)	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

  </tbody>
</table>
                                </div>
                            </div>
                        </div>

                        <div class="card">
                            <div id="headingThree" role="tab" class="card-header">
                                <h4 class="card-title">
                                    <a style="color: blue;" aria-controls="collapseThree" aria-expanded="false" href="#collapseThree"
                                       data-parent="#accordion" aria-expanded="true"  data-toggle="collapse"  class="collapsed">
                                       COLOMBO FORT - HATTON 
                                    </a>
                                </h4>
                            </div>
                            <div aria-labelledby="headingThree" role="tabpanel" class="panel-collapse collapse"
                                 id="collapseThree">
                                <div class="card-body">
                                <table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Depature</th>
      <th scope="col">Time</th>
      <th scope="col">Arrival</th>
      <th scope="col">Time</th>
      <th scope="col">Train no:</th>
      <th scope="col">Train type</th>
      <th scope="col">Class </th>
      <th scope="col">A / C</th>
      <th scope="col">Fare</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    
      <td> Colombo fort</td>
      <td>	06:00 hrs</td>
      <td> Hatton</td>
      <td>09:15 hrs</td>
      <td>1005</td>
      <td> Express	</td>
      <td>1st Class </td>
      <td> Yes	</td>
      <td>£11.00 </td>
    </tr>
    				
    <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
    				
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>

  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>

  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td> Hatton</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
  
  </tbody>
</table>
                                </div>
                            </div>
                        </div>

                        <div class="card">
                            <div id="heading_04" role="tab" class="card-header">
                                <h4 class="card-title">
                                    <a style="color: blue;" aria-controls="collapse_04" aria-expanded="false" href="#collapse_04"
                                       data-parent="#accordion" aria-expanded="true"  data-toggle="collapse"  class="collapsed">
                                       COLOMBO FORT - NANUOYA
                                    </a>
                                </h4>
                            </div>
                            <div aria-labelledby="heading_04" role="tabpanel" class="panel-collapse collapse"
                                 id="collapse_04">
                                <div class="card-body">
                                <table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Depature</th>
      <th scope="col">Time</th>
      <th scope="col">Arrival</th>
      <th scope="col">Time</th>
      <th scope="col">Train no:</th>
      <th scope="col">Train type</th>
      <th scope="col">Class </th>
      <th scope="col">A / C</th>
      <th scope="col">Fare</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td>Nanuoya</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
          
  <tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
          
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Nanuoya</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

  </tbody>
</table>


 
            
                                </div>
                            </div>
                        </div>

                        <div class="card">
                            <div id="heading_05" role="tab" class="card-header">
                                <h4 class="card-title">
                                    <a style="color: blue;" aria-controls="collapse_05" aria-expanded="false" href="#collapse_05"
                                       data-parent="#accordion" aria-expanded="true"  data-toggle="collapse"  class="collapsed">
                                       COLOMBO FORT - HAPUTALE
                                    </a>
                                </h4>
                            </div>
                            <div aria-labelledby="heading_05" role="tabpanel" class="panel-collapse collapse"
                                 id="collapse_05">
                                <div class="card-body">
                                <table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Depature</th>
      <th scope="col">Time</th>
      <th scope="col">Arrival</th>
      <th scope="col">Time</th>
      <th scope="col">Train no:</th>
      <th scope="col">Train type</th>
      <th scope="col">Class </th>
      <th scope="col">A / C</th>
      <th scope="col">Fare</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    
    <td> Colombo fort</td>
    <td>	06:00 hrs</td>
    <td>Haputale</td>
    <td>09:15 hrs</td>
    <td>1005</td>
    <td> Express	</td>
    <td>1st Class </td>
    <td> Yes	</td>
    <td>£11.00 </td>
  </tr>
          
  <tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Haputale</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
          
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Haputale</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Colombo fort</td>
  <td>	06:00 hrs</td>
  <td>Haputale</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

  </tbody>
</table>
            
         
                                </div>
                            </div>
                        </div>

                        <div class="card" >
                            <div id="heading_06" role="tab" class="card-header" >
                                <h4 class="card-title" >
                                    <a aria-controls="collapse_06" aria-expanded="false" href="#collapse_06"
                                       data-parent="#accordion" aria-expanded="true"  data-toggle="collapse"  class="collapsed" style="color: blue;">
                                     NANUOYA-BADULLA <h4 style="color: red; text-transform:lowercase;">Please note that trains have only limited availability<h4>
                                    </a>
                                </h4>
                            </div>
                            <div aria-labelledby="heading_06" role="tabpanel" class="panel-collapse collapse"
                                 id="collapse_06">
                                <div class="card-body">

                                <table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Depature</th>
      <th scope="col">Time</th>
      <th scope="col">Arrival</th>
      <th scope="col">Time</th>
      <th scope="col">Train no:</th>
      <th scope="col">Train type</th>
      <th scope="col">Class </th>
      <th scope="col">A / C</th>
      <th scope="col">Fare</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    
  <td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
          
  <tr>
  
  <td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
          
<tr>
  
<td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
<td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
<td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

<tr>
<td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

<tr>
  
<td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
<td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>
<tr>
  
  <td> Nanuoya</td>
  <td>	06:00 hrs</td>
  <td>Badulla</td>
  <td>09:15 hrs</td>
  <td>1005</td>
  <td> Express	</td>
  <td>1st Class </td>
  <td> Yes	</td>
  <td>£11.00 </td>
</tr>

  </tbody>
</table>









                                </div>
                            </div>
                        </div>
                        <br>

                        <img src ="images/Sri-Lanka-train-map.jpg" style="width: 400px; height: 500px; margin-left: 50px; margin-bottom: 3%;">


                    </div>


                </div>


            </div>
        </div>
    </div>

                      

                       
                     
                      
                      
                     </div>
                                
                        </div>


                    </div>


                </div>


            </div>
        </div>
    </div>

 <?php include('footer.php');?>
    
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
<script>window.jQuery || document.write('<script src="assets1/js/jquery/jquery-3.3.1.min.js">\x3C/script>')</script>
<script src="assets1/bootstrap/js/bootstrap.bundle.js"></script>
<script src="assets1/js/vendors.min.js"></script>

<!-- include custom script for site  -->
<script src="assets1/js/main.min.js"></script>




</body>
</html>